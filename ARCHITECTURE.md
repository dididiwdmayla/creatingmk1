...
**Cobertura de viewport** — Aura e Grão montam o próprio elemento-raiz com `position: fixed; inset: 0` (nunca `absolute`: um `absolute inset-0` sem ancestral posicionado fica preso à altura do bloco inicial do documento — na prática, some depois do primeiro scroll) e `z-index` **negativo** (`-z-10`), pra ficar sempre atrás do conteúdo normal da demo — inclusive seções sem `position` (que, com `z-index: auto`, pintam DEPOIS de um elemento posicionado do mesmo nível se ele não tiver z-index negativo) e seções com fundo próprio (`--d-bg-alt` etc., que simplesmente cobrem o efeito por cima). `pointer-events: none` garante que nunca bloqueia clique. `position: fixed` sempre cria stacking context próprio, então o `-z-10` só precisa vencer o stacking context RAIZ (nem `Skin.tsx` nem `demo/[leadId]/page.tsx` criam um próprio — sem `transform`/`filter`/`opacity<1` no meio do caminho).
