...
scripts/
  ...
  video-portfolio.mjs               # ✅ LOCAL, nunca em CI/Vercel: grava VÍDEO das demos descendo (material de portfólio pra TikTok/Reels/Kwai). É o avesso do motor de capturas — as animações ficam RODANDO —, converte o .webm do Playwright em .mp4 pelo ffmpeg do sistema e reprova a rodada se a gravação estampar o selo ou disparar beacon de visita (ver "Vídeo de portfólio das demos")
src/
...
### Vídeo de portfólio das demos (`scripts/video-portfolio.mjs`)

Gravação em vídeo da demo descendo do topo ao rodapé, pra material de divulgação em TikTok/Reels/Kwai. **Local, sempre**: `CI`/`GITHUB_ACTIONS`/`VERCEL` no ambiente abortam antes de subir servidor ou navegador. Não tem workflow, não tem rota, não é chamado por nada — é digitado à mão, e o resultado é olhado antes de virar post.

**É o AVESSO do motor de capturas, e é essa a única razão de ser um script separado.** `capturas.mjs` congela o relógio das animações (`congelarAnimacoes`), prende o LED numa fase fixa (`assentarLed`) e some com o cromo (`neutralizarCromo`) — tudo pra que a FOTO não saia no meio do voo. Aqui efeito de fundo, LED e revelação de seção precisam estar RODANDO, senão não há o que filmar; nenhuma das três funções é chamada. De `capturas/dom.mjs` vem só `esperarTextoEstavel` — o resto do módulo existe pra congelar.

**A preparação não varre a página**, e essa é a diferença que decide se o vídeo tem animação ou não. A varredura de `prepararPagina` dispara o lazy-load E as revelações de `whileInView`, que são `once: true`: uma vez disparadas não voltam. Numa foto é exatamente o que se quer; num vídeo a gravação chegaria com tudo já revelado. O lazy-load é forçado pelo outro caminho, o que não toca o observador de entrada na viewport: `loading="eager"` em toda imagem (as skins usam `next/image`, que emite `loading="lazy"` nativo) + varredura HORIZONTAL dos trilhos roláveis, sem mexer no scroll (o retorno traz `rolagem`, que tem de ser 0). A descida é por `requestAnimationFrame` com easing ease-in-out cúbico, `behavior: "instant"` em cada passo (`imobiliaria` e `multimarcas` declaram `html { scroll-behavior: smooth }` — mesmo motivo documentado em `rolarAteSecao`) e o fundo do documento recalculado A CADA QUADRO, porque revelação e lazy-load mudam a altura durante a descida.

**O que o `recordVideo` do Playwright faz e não faz — medido por sonda**, e é daí que saem os números do script:

- **`recordVideo.size` NÃO ESCALA.** O pipeline interno é `pad`+`crop`, não `scale`: pedir 1080×1920 com viewport menor entrega o site num retângulo no canto superior esquerdo e cinza no resto. Por isso `size` é sempre IGUAL ao viewport e a resolução final sai do **ffmpeg**.
- **`deviceScaleFactor` NÃO CHEGA AO VÍDEO.** O quadro sai em pixel CSS — dpr 1, 2 e 3 deram exatamente o mesmo tamanho de arquivo. O dpr ainda vale alguma coisa: `window.devicePixelRatio` na página É 2, então o `next/image` escolhe o asset 2× e o que entra no quadro é a versão boa da foto. Mas ele não acrescenta um pixel ao vídeo.
- **O fps é fixo em 25** e o Playwright não expõe controle. O mp4 mantém os 25 (as três plataformas aceitam); converter pra 30 seria duplicar quadro, e numa panorâmica linear a duplicação aparece como tremidinha.
- **`prefers-reduced-motion` é `false` por padrão**, mas o contexto pina `reducedMotion: "no-preference"` mesmo assim. Default é coisa que muda de versão, e no dia em que mudasse as skins pulariam `TypewriterText`, `SectionReveal`, `Parallax` e a intro por completo (elas consultam a media query direto): o vídeo sairia com a página parada e nada denunciando o porquê.

**Celular em 540×960, e não nos 390×844 de um iPhone.** O quadro das três plataformas é 9:16 (1080×1920); 390×844 é 19,5:9, e levar um ao outro obrigaria a escolher entre distorcer 11% na horizontal, cortar 18% da altura ou pôr barra dos dois lados. 540×960 é 9:16 exato, então a conversão é uma multiplicação por 2 — sem barra, sem corte, sem distorção. E como o vídeo sai em pixel CSS, a largura do viewport É a resolução real do material: 540 px verdadeiros ampliados 2×, contra 390 ampliados 2,77×. 540 continua abaixo do breakpoint `sm:` do Tailwind (640px), então é o MESMO layout de celular que um aparelho de 390 mostraria. O desktop (`--desktop`) é 1920×1080 em dpr 1: pixel CSS ao pixel de saída, sem escala nenhuma na conversão.

**ffmpeg do sistema, não o do Playwright.** Quem roda Playwright neste repo já tem um ffmpeg no disco (`$PLAYWRIGHT_BROWSERS_PATH/ffmpeg-*`), e ele NÃO serve: é compilado com `--disable-everything`, só mjpeg→vp8/webm, sem libx264 e sem muxer mp4 — é justamente o que grava o `.webm` que o script recebe. A checagem vai até o encoder (`ffmpeg -encoders | libx264`) pra o erro ser "instale um ffmpeg completo" e não uma mensagem de codec lá na frente. A saída vai pra `saida-video/` (no `.gitignore`) e o `.webm` é apagado.

**Corte de cabeça.** O Playwright começa a gravar no instante em que a página é criada e não tem pausar/retomar, então o `.webm` sempre traz a navegação e a preparação antes do vídeo de verdade. O deslocamento é medido por relógio de parede entre `newPage` e o fim do assentamento, com um colchão de 300ms: o relógio do `.webm` começa no primeiro quadro que o Chromium emite, não na criação da página, então errar pra menos é inofensivo (sobra um pedaço do assentamento, visualmente idêntico à pausa do topo) e errar pra mais comeria o respiro de abertura.

**Não contaminar o rastreio** — o mesmo caminho de exclusão do motor de capturas (não existe header de segredo nem user-agent de pré-visualização neste repo): (1) contexto do navegador **sem cookie nenhum** na rota pública, então `classificarVisitaInterna` dá `false` e o selo "Vendo como membro" não é renderizado; (2) URL **sem `?t=`**, então `DemoPage` não chama `registrarVisitaDemo` (nada entra em `lead.demoVisitas`) e nem monta o `<VisitaTracker>`. As duas viram medida e **reprovam a rodada** em vez de avisar — não sai mp4, e o `.webm` parcial é apagado: URL com `?t=` aborta antes de abrir o navegador; qualquer cookie no contexto público aborta antes de navegar; o selo no DOM renderizado aborta logo depois do `goto`; um espião de `navigator.sendBeacon` instalado por `addInitScript` **registra e barra** (não repassa — uma regressão futura não chega a escrever no lead); `ctx.route` barra a mesma coisa na rede; e `forcarDescarga` dispara `visibilitychange`→hidden e `pagehide` À MÃO com a página ainda viva, que é o que transforma "o beacon não disparou" de esperança em medida — os dois eventos reais só acontecem quando a página fecha, e aí não dá mais pra perguntar nada a ela. **Verificado na rota pública de verdade** (lead semeado num fake DB local, ver o cabeçalho de `qa-editor.mjs`): gravação = 0 cookie, 0 selo, 0 beacon, `demoVisitas` intacta; e o controle negativo — a MESMA demo aberta COM o cookie de sessão e COM o `?t=` vigente — mostra selo presente, 2 beacons e `demoVisitas` indo a 1 com `interna: true`. O rastreio está vivo; é o caminho de exclusão que o mantém fora.

**Uso.** Alvo (escolha um): `--skin=<skinId>` (harness `/interno/demo-qa`, com `intro=0` pelo mesmo motivo das capturas), `--lead=<placeId>` (rota pública, sem token) ou `--todas`. Variações, só com `--skin`/`--todas`: `--tema=<presetId>` e `--efeito=<id>|nenhum` — com `--lead` elas são **recusadas**, não ignoradas, porque a demo do lead traz o tema e o efeito que o editor salvou e sobrescrevê-los mostraria no vídeo um site que o lead nunca viu. Resto: `--desktop` (acrescenta o passe de 1920×1080), `--duracao=<ms>` (default 3500), `--saida=<dir>`, `--sem-build`. Nomes: `<alvo>-mobile.mp4` e `<alvo>-desktop.mp4`; com `--tema`/`--efeito` a variação entra no nome, pra três presets da mesma skin não sobrescreverem um ao outro.

`--efeito` é **conferido na página** (a camada se anuncia em `data-d-efeito-camada` com o id dentro), então um id fora do registro reprova a gravação em vez de gerar um vídeo sem o efeito pedido. `--tema` **não tem como ser conferido**: o preset não deixa marca no DOM e `getTheme` cai no `themeDefault` da skin quando o id não bate — um erro de digitação ali grava no tema default, calado. Os ids válidos estão em `themePresets`, em `src/components/demos/<nicho>/themes.ts`.

**Lote em série, nunca em paralelo** — ao contrário do motor de capturas, que roda celular e desktop juntos. Lá o produto é um PNG e disputar CPU só atrasa; aqui o produto é uma sequência de quadros, e um segundo Chromium rolando uma página com efeito de fundo rouba exatamente os quadros que o vídeo existe pra mostrar. Um alvo que reprova não derruba o lote (perder 7 skins boas por causa da 8ª é pagar o build de novo por nada): o resumo final lista os reprovados e define o código de saída.

**A lista de skins do `--todas`** sai das chaves de `ANCORAS_PADRAO` (`capturas/padrao.mjs`), mesmo precedente do `capturas.mjs`: o registro é TypeScript e o script não compila TS. **O acoplamento é real — skin nova sem entrada em `ANCORAS_PADRAO` não aparece no `--todas`** (nem nas capturas). De propósito NÃO consulta `/api/config`, ao contrário do motor de capturas: lá "lista vazia" é marcação legítima de "não capturar esta skin", e uma skin que ninguém quer fotografar continua valendo como material de portfólio.

**Estado da última rodada**: `--todas --desktop` gerou as 16 (8 skins × 2 telas), nenhuma reprovada, todas as guardas de rastreio limpas e nenhum `.webm` sobrando. Um número que vale registrar: a descida roda a ~60 q/s no celular em toda skin, e no desktop cai pra 15–26 q/s (o pior é `tatuagem-pigmento-vivo`, 15,3) — é o custo real da página em 1920×1080, e como o vídeo é gravado a 25 fps, é ali que a panorâmica de desktop fica menos fluida que a de celular.
