...
- **Token continua sendo o portão de entrada**: sem `?t=` reconhecido, nada é registrado (comportamento de antes, intencional — "Copiar link"/"Abrir demo" da ficha usam a URL sem token de propósito, pra não virar ruído na timeline a cada preview). Uma visita com token, sem sessão e sem marcador ainda é registrada — só fica `interna: false`.
