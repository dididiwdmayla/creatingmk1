...
| `/api/search` | POST | `{ nicho?, subNicho?, regiao?, nome?, quantidade? (1–40), qualificada?, soComTelefone? }` (nicho/regiao default: config; `soComTelefone` exige `qualificada: true`) | `200 { criados, existentes, leads[], busca, paginas, regiaoResolvida, validos?, aviso? }` · `400` · `429 quota_exceeded` · `502 places_error` | Geocoding (com cache) + Text Search · **geocoding** + **textSearch** ou **textSearchEnterprise** |
...
- **`soComTelefone: true`, checkbox "Só com telefone"** (só disponível/aceito com `qualificada: true`, já que telefone só vem definitivo na qualificada): descarta, página a página, todo resultado sem `nationalPhoneNumber`/`internationalPhoneNumber` — não entra em `places` nem é upsertado em `/leads`, nem conta para a `quantidade` pedida. Continua paginando (mesma cota-por-página e limite de 3 páginas do Text Search) até juntar `quantidade` leads NOVOS com telefone ou os resultados se esgotarem. A resposta traz `validos` (quantos passaram no filtro, novos + já existentes) e a UI monta "X válidos em Y páginas" a partir de `validos`/`paginas`.
...
- **`(app)/` (route group)**: layout com nav inferior fixa (Painel/Leads/Config) + botão Sair; todas as páginas autenticadas vivem aqui.
  ...
  - **`/leads`**: form de nova busca (`POST /api/search`, trata `quota_exceeded`/`places_error`/`aviso` parcial com mensagem específica; campos nicho/sub-nicho/região/nome, quantidade 1–40, checkbox "Só sem site" com "Só com telefone" condicional — só aparece/é enviado com a qualificada ativa, relata "X válidos em Y página(s)" — e auto-enriquecimento dos primeiros N ≤ 5) + filtros (status/site/telefone/favoritos) + lista com **agrupamento colapsável por busca** (grupos iniciam **colapsados**; toggle, header com dot da cor + nome + contagem; estado expandido/colapsado persiste na URL junto dos outros filtros; lead em várias buscas aparece em cada grupo; "Sem busca" agrupa o resto). Cada card (`LeadCard`) tem estrela de favorito e notas editáveis inline — sem abrir a ficha — além dos dots de cor das buscas e destaque "sem site (lead quente)". Aceita `?buscaId=` na URL (via `useSearchParams`, com Suspense) para mostrar só os leads de uma busca (aí a lista é plana), com chip de filtro e botão limpar. Campo região geocodifica **no blur** (cache por texto no cliente + cache permanente no servidor) e mostra "Buscando em: X" com ✓ antes do botão Buscar — região não confirmada mostra aviso e bloqueia a busca. Ao voltar da ficha, a posição de scroll é restaurada em `useLayoutEffect` (antes do paint) com um overlay de sweep cobrindo o carregamento, e o `Link` do card não força scroll-to-top ao navegar (`scroll={false}`) — evita perder a posição salva.
