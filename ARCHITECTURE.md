...
3. **A estrutura é dado, não código**: a skin renderiza suas seções pela **ordem efetiva** (`estrutura.ts`: `ordemSecoes` filtrado contra o contrato `SkinDefinition.secoes`, fixas no lugar, ids desconhecidos ignorados) e pula as `oculta`. Uma seção reordenável **não listada** em `ordemSecoes` entra antes da primeira seção listada que a sucede no CONTRATO (a ordem default de `SkinDefinition.secoes`), e só no fim se nenhuma sucede — não "sempre no fim" (regra trocada na etapa 1 da migração da multimarcas para variantes, docs/plano-multimarcas.md §5: sem isso, uma seção NOVA no meio de um contrato que cresce nasceria depois do rodapé em toda demo salva antes dela existir). Numeração de seção ("01 / FILOSOFIA") é recalculada pela ordem visível — reordenar/ocultar nunca deixa número furado. Cada texto/imagem da skin carrega **`data-demo-slot="<caminho do slot>"`** (ex.: `secoes.hero.titulo`, `servicos.0.preco`, `imagens.hero`) — atributo inerte na demo pública que o editor usa para o mapa clique-no-preview → campo-do-painel.
...
**`contactadoForaDaFila` — o buraco do clique manual sem status.** O botão de WhatsApp da ficha e de `/hoje` (`registrarSeloContato`, `lib/leads/repo.ts`) grava `seloContato` + soma em `registrosEnvio` a cada clique, **sem tocar `status`** — só o botão "Contactado", à parte, faz essa transição. Sem checagem própria, um lead contactado à mão pela ficha ficava `status: "novo"` PARA SEMPRE e voltava candidato à fila automática todo santo dia, como se nada tivesse acontecido (já aconteceu: leads antigos receberam mensagem de novo). `motivoEstrutural` agora barra também por `seloContato` presente, `registrosEnvio` não vazio ou `contato.primeiroContatoEm` preenchido (o carimbo da transição "novo → contactado", `STATUS_STAMPS` em `lib/leads/repo.ts` — cobre doc gravado por fora da regra normal, migração ou edição direta).

A checagem vem **logo depois de `status`**, e a ordem importa: o envio confirmado PELA fila também grava selo e registro (`confirmarEnvio`, `lib/fila/confirmar.ts`), na MESMA transação que move `status` de "novo" para "contactado" — os dois nascem atomicamente juntos. Checando `status` primeiro, esse lead já sai por "status"; checar a ordem inversa rotularia o envio automático como "contactado fora da fila", o que seria falso. Só quem tem selo/registro/carimbo SEM `status` ter mudado — exatamente o clique manual — cai na razão nova.

**Tentativas esgotadas (`filaParado`) fica FORA do diagnóstico estrutural, de propósito**: por construção, um envio "enviado" já reprova antes em `status` (a confirmação move `novo → contactado` na mesma transação, com selo e registro incluídos) e "inválido" já reprova antes em `telefoneInvalido` (mesma transação) — só "tentativas esgotadas" sobra, e esse caso já tem vitrine própria na ficha do lead. Duplicá-lo no pool confundiria duas fontes da mesma informação. Essa garantia (selo/registro sempre vêm com `status` já mudado) só vale para quem passou pela confirmação da fila — o clique manual, sem doc de envio nenhum, é o que `contactadoForaDaFila` cobre, na etapa estrutural, não aqui.

**A contagem de elegíveis cai depois desta mudança — é o comportamento correto, não regressão**: todo lead contactado manualmente e nunca movido para "Contactado" agora sai do pool. O painel ("A VISÃO da fila" adiante) mostra a peneira nova com rótulo explícito, para o operador não ver o número cair sem saber por quê.

**Não muda `/api/fila/proximo` nem `/api/fila/confirmar`.** A checagem é inteiramente do lado do FILTRO (`motivoEstrutural`), não da confirmação — os contratos das duas rotas (chaves fixas, `runTransaction` de `confirmarEnvio`) continuam exatamente como estavam; os testes de contrato de ambas continuam passando byte a byte.
...
**TERCEIRO GATILHO: abrir o painel "Respostas pendentes"** (`GET /api/config/fila/respostas`) — e ele conserta um problema de PRODUÇÃO, não de teste. Os dois gatilhos acima dependem do APARELHO, e a macro de envio só chama `/proximo` com o celular parado, bloqueado e ocioso há mais de dez minutos; o Radar não tem visão nenhuma do uso do celular. Consequência: um lead de VERDADE que responde enquanto o operador está com o telefone na mão ficava sem rascunho até o aparelho ficar ocioso. A pessoa que abre o painel está justamente perguntando "o que chegou?" — é o momento certo de esvaziar, e é ela quem espera pelo resultado. Os dois gatilhos antigos continuam valendo; este é somado, não substitui.

MESMA função (`flushGruposMaduros`), nunca uma cópia, e com as mesmas três garantias: grupo ainda dentro da janela de agrupamento NÃO é gerado antes da hora (`reivindicarGrupoMaduro` só entrega o que venceu); falha de IA não impede o painel de carregar (a função nunca lança — o painel responde 200 com o que houver, e o grupo que falhou volta ao pendente marcado e retentável); e o custo é o de sempre, uma geração por grupo MADURO, com reserva de cota antes do request. `requireAdmin` roda ANTES do flush: um membro toma 403 sem gastar cota.

**A LINHA DE ESTADO do painel** nasce do mesmo lugar. Entre a mensagem chegar e o rascunho existir corre a janela de silêncio, e até aqui não havia sinal NENHUM na tela de que algo tinha chegado: "Nenhuma resposta esperando" cobria tanto "a captura no celular falhou" quanto "chegou, a janela ainda está aberta" — duas causas muito diferentes com a mesma cara. A rota passa a devolver, junto da lista, `aguardando` (quantos grupos têm mensagem esperando e nenhum erro) e `comErro` (os grupos cuja geração falhou, com nome do lead, quantas mensagens, quantas tentativas e o motivo — NUNCA o texto das mensagens: a contagem basta, e o corpo é conversa privada). O resumo do cabeçalho FECHADO carrega as duas contagens, porque recolhido é o único estado visível do painel.

A divisão entre as duas contagens é pelo ERRO, não pela maturidade, e é por isso que `resumirGruposPendentes` não precisa de relógio: o resumo é lido DEPOIS do flush, então todo grupo maduro que sobrou é um que a geração não conseguiu produzir — ele tem `ultimoErro`. Lido antes, mostraria como "aguardando" exatamente os grupos que acabaram de virar rascunho.
...
**Verificação visual:** `node scripts/qa-plataforma.mjs --so=comercial` captura o painel PREENCHIDO (seis linhas de texto livre) e VAZIO × celular e desktop × escuro e claro. O estado vazio é o estado INICIAL de qualquer instalação — é ele que faz o rascunho pedir preenchimento em vez de inventar —, e o passo cobra que as duas regras duras continuem escritas na tela justamente ali, sem documento, onde elas são o único freio.

E a imagem cobrou um defeito que nenhum teste pegaria: a caixa tinha altura FIXA (`rows={10}`), e no celular o documento de seis linhas terminava com meia fileira de letras fatiada na borda de baixo. É o mesmo defeito que a caixa do rascunho já tinha resolvido, e pesa mais aqui: este painel existe para o operador ACRESCENTAR uma linha a cada pergunta nova, e uma caixa que esconde o que ele acabou de escrever trabalha contra isso. A caixa passou a CRESCER com o conteúdo (mesmo efeito de `LinhaResposta`), sem teto — não há lista de irmãos para empurrar, e este é o único lugar onde este texto existe.
...
**Verificação visual:** `node scripts/qa-plataforma.mjs --so=respostas` captura o painel em seis estados × celular e desktop × temas escuro e claro. Os estados: **cheia** (um grupo de três mensagens, e um rascunho longo com quebras de linha — o pior caso de layout da caixa editável), **linha de estado** (uma conversa RECEBIDA ainda na janela de agrupamento e uma que FALHOU na geração, nomeada e retentável), **editada**, **simulação com resultado**, **contexto enviado expandido** e **VAZIA**, que cobra o painel ENCOLHER (−1018px no celular, −875px no desktop) em vez de trocar a lista por um vão.

Os dois estados novos não são encenados: o grupo "na janela" está mesmo dentro dela e o grupo "com erro" está mesmo maduro — abrir o painel roda o flush de verdade, e quem recusa a geração do segundo é um STUB de IA local (parte do patch temporário: `gemini.ts` passa a ler o endpoint de `RADAR_FAKE_AI_URL`). O caminho inteiro é exercido; o que se substitui é o MODELO, não o código do app. Duas armadilhas de tempo saíram daí: a janela default de 45s é curta demais para o laço (entre repor o carimbo do grupo e o navegador carregar a /config passam segundos imprevisíveis, e o grupo amadurecia no meio do caminho), então a semeadura usa 10 minutos — configuração legítima do campo; e o repositor do grupo reescreve a MENSAGEM inteira, não só a data, porque depois de um flush o doc fica como âncora vazia. O aferidor de "slot com caixa zerada" passou a ignorar o corpo de bloco FECHADO (`[data-corpo="fechado"]`, a mesma regra do `--so=paineis`): "Simular mensagem" é um bloco de nível 3 dentro deste painel, e fechado ele virava uma parede de falso positivo. O passo existe por um motivo a mais que os vizinhos: **a ação muda com o aparelho**, e o Chromium do laço se apresenta como desktop — sem forçar um agente Android (`contextoLogado` ganhou `userAgent`), a captura do "celular" mostraria o caminho do desktop, provando o contrário do que existe para provar. No Android o passo exige âncora `intent://` com o pacote do Business, e reprova se o pacote do WhatsApp COMUM aparecer mirado; no desktop exige o inverso — nenhuma âncora, o aviso e os botões de copiar. Dois aferidores que só a tela real faz: o texto extraído do `href` tem que ser IDÊNTICO ao valor da caixa editável (com `%0A` nas quebras), e depois de digitar um texto com `#` e `;` dentro o URI ainda tem que ter exatamente um `#Intent;` e terminar em `;end`. O unitário prova a função; este prova que o que o operador digitou é o que entra no link.
